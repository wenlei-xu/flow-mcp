...
### `GFLOW_CLI_GENERATION_RATE_CAPACITY` / `GFLOW_CLI_GENERATION_RATE_REFILL_SECONDS`

**What:** Process-wide MCP spend-safety token bucket for image and video generation.
The capacity controls the initial burst; the refill setting controls how often one
new generation token becomes available.
**Default:** `8` and `20` seconds (8-request burst, then one request every 20 seconds)
**Important:** This is only a local pacing guard. It does not increase Google's
quota, bypass reCAPTCHA/WAF, or create additional account/Profile concurrency. A
single persistent Chrome Profile remains serialized by its lease.
**Example:**

```bash
GFLOW_CLI_GENERATION_RATE_CAPACITY=4 \
GFLOW_CLI_GENERATION_RATE_REFILL_SECONDS=30 \
gflow serve
```
